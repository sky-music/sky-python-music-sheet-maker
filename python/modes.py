...
class InputMode(Enum):
    ...
    def __init__(self, number, short_desc, long_desc):
...
class RenderMode(Enum):
    HTML = (1, 'HTML visual sheet', 'text/html', '.html')
    SVG = (2, 'SVG visual sheet', 'image/svg+xml', '.svg')
    PNG = (3, 'PNG visual sheet', 'image/png', '.png')
    MIDI = (4, 'Midi song file', 'audio/midi', '.mid')
    SKYASCII = (5, 'song in Sky notation', 'text/plain', '_sky.txt')
    ENGLISHASCII = (6, 'song in English notation', 'text/plain', '_english.txt')
    JIANPUASCII = (7, 'song in Jianpu notation', 'text/plain', '_jianpu.txt')
    DOREMIASCII = (8, 'song in do-re-mi notation', 'text/plain', '_doremi.txt')

    def __init__(self, number, short_desc, mime_type, extension):
