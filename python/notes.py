    ...
    def render_in_html(self, width, chord_skygrid, position, instrument_type, note_index, harp_is_empty):
        ...
        try:
            ...
            if harp_is_empty and not(note_index == 7):
                note_render = '<svg class=\"' + self.svgclass + ' unhighlighted empty ' + instrument_type + '-button-' + str(note_index) + '\" xmlns=\"https://www.w3.org/2000/svg\" width=\"' + str(width) + '\" height=\"' + str(width) + '\" viewBox=\"0 0 91 91\">\n'
...
class NoteCircle(Note):   
    ...
    def get_note_svg(self, highlighted_classes):
         return '<circle cx="45.4" cy="45.4" r="25.5" class="instrument-button-icon ' + ' '.join(highlighted_classes).rstrip() + '"/>'
...
class NoteDiamond(Note):   
    ...
    def get_note_svg(self, highlighted_classes):
         return '<rect x="22.6" y="22.7" transform="matrix(-0.7071 -0.7071 0.7071 -0.7071 45.3002 109.5842)" width="45.4" height="45.4" class="instrument-button-icon ' + ' '.join(highlighted_classes).rstrip() + '"/>'
