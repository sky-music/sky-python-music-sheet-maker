...
#        return note_render
    ...
    def render_in_html(self, width, chord_skygrid, position, instrument_type, note_index, harp_is_empty):

        try:
            note_states = chord_skygrid[position]
        except KeyError:
            highlighted_classes = []

            if harp_is_empty and not(note_index == 7):
                note_render = '<svg class=\"note-root unhighlighted empty ' + instrument_type + '-button-' + str(note_index) + ' \" xmlns=\"https://www.w3.org/2000/svg\" width=\"' + str(width) + '\" height=\"' + str(width) + '\" viewBox=\"0 0 91 91\">\n'
                note_render += '<circle cx="45.4" cy="45.4" r="12" class="instrument-button-icon unhighlighted empty' + ' '.join(highlighted_classes).rstrip() + '"/>'
                note_render += '</svg>\n'
            else:
                if harp_is_empty and note_index == 7:
                    # Display the circle in the middle for blank icons
                    ...
                    note_render += '<circle cx="45.4" cy="45.4" r="20" class="instrument-button-icon unhighlighted ' + ' '.join(highlighted_classes).rstrip() + '"/>'
                    note_render += '</svg>\n'
                else:
                    note_render = '<svg class=\"note-root ' + instrument_type + '-button-' + str(note_index) + ' \" xmlns=\"https://www.w3.org/2000/svg\" width=\"' + str(width) + '\" height=\"' + str(width) + '\" viewBox=\"0 0 91 91\">\n'
...
class NoteCircle:
    ...
    def render_in_html(self, width, chord_skygrid, position, instrument_type, note_index, harp_is_empty):
        ...
        try:
            ...
            if harp_is_empty:
                note_render = '<svg class=\"note-circle unhighlighted empty ' + instrument_type + '-button-' + str(note_index) + ' \" xmlns=\"https://www.w3.org/2000/svg\" width=\"' + str(width) + '\" height=\"' + str(width) + '\" viewBox=\"0 0 91 91\">\n'
                note_render += '<circle cx="45.4" cy="45.4" r="12" class="instrument-button-icon unhighlighted empty' + ' '.join(highlighted_classes).rstrip() + '"/>'
                note_render += '</svg>\n'
            else:
...
class NoteDiamond:
    ...
    def render_in_html(self, width, chord_skygrid, position, instrument_type, note_index, harp_is_empty):
        ...
        try:
            ...
            if harp_is_empty:
                note_render = '<svg class=\"note-diamond unhighlighted empty ' + instrument_type + '-button-' + str(note_index) + ' \" xmlns=\"https://www.w3.org/2000/svg\" width=\"' + str(width) + '\" height=\"' + str(width) + '\" viewBox=\"0 0 91 91\">\n'
                note_render += '<circle cx="45.4" cy="45.4" r="12" class="instrument-button-icon unhighlighted empty ' + ' '.join(highlighted_classes).rstrip() + '"/>'
                note_render += '</svg>\n'
            else:
