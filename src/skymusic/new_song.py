...
class Song():
    ...
    def write_svg(self, css_mode=CSSMode.EMBED, css_path='css/main.css', rel_css_path='../css/main.css', start_row=0, buffer_list=None):
        ...
        if filenum == 0:
            song_header += f"\n<text x=\"{x :.2f}\" y=\"{y :.2f}\" class=\"title\">{self.meta['title'][1]}</text>"           
            
            for k in self.meta:
                if k != 'title':
                    y += 2 * self.SVG_text_height
                    song_header += f'\n<text x="{x :.2f}" y="{y :.2f}" class="headers">{self.meta[k][0]} {self.meta[k][1]}</text>'
                    
        else:
            song_header += f"\n<text x=\"{x :.2f}\" y=\"{y :.2f}\" class=\"title\">{self.meta['title'][1]} (page {(filenum + 1)})</text>"
